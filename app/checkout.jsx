...
export default function CheckoutScreen() {
  ...
  const gst = Math.round(servicePrice * (masterAdminData?.serviceGST / 100));
  const convenienceFee = masterAdminData?.platformFee || 0;
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      Toast.show({
        ...
        text2: `Saved ₹${res.data.discount}`,
      });
    } catch (err) {
      Toast.show({
        type: "error",
        text1: err.response?.data?.error || "Invalid coupon",
      });
      setDiscount(0);
  ...
  const handleAddAddress = async () => {
    ...
    try {
      ...
      Toast.show({
        type: "error",
        text1: err.response?.data?.error || "Failed to save address",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        >
          <Image
            source={{ uri: parsedService?.imageUrl }}
        ...
        <View style={{ padding: 16 }}>
          <Text style={{ fontWeight: "600", color: "#666", marginBottom: 12 }}>
            DELIVERY ADDRESS
          </Text>
          <TouchableOpacity
            style={{
          ...
          >
            {currentUser?.vehicles && currentUser?.vehicles !== null ? (
              <>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: "600" }}>
                    RC: {currentUser?.vehicles?.registrationNumber}
                  </Text>
                  <Text style={{ fontSize: 13, color: "#666" }}>
                    Brand: {currentUser?.vehicles?.make}
                  </Text>
                  <Text style={{ fontSize: 13, color: "#666" }}>
                    Model: {currentUser?.vehicles?.model}
                  </Text>
                  <Text style={{ fontSize: 13, color: "#666" }}>
                    Segment: {currentUser?.vehicles?.type}
                  </Text>
                </View>
              </>
            ) : (
          ...
          {availableCoupons.length > 0 && (
            <View style={{ marginBottom: 16 }}>
              ...
              >
                ...
                <Ionicons
                  name={couponDropdownOpen ? "chevron-up" : "chevron-down"}
                  size={20}
                  color="#0369a1"
                />
              </TouchableOpacity>
        ...
        <View style={{ padding: 16, marginBottom: 20 }}>
          ...
          >
            ...
            >
              <Text>GST ({masterAdminData?.serviceGST || 0}%)</Text>
              <Text>₹{gst}.00</Text>
            </View>
            <View
              style={{
              ...
              {addresses.map((addr) => (
                <View
                  ...
                  style={{
                    ...
                    borderBottomWidth: 1,
                    borderBottomColor:
                      selectedAddress?._id === addr._id ? "#EF4444" : "#f0f0f0",
                    backgroundColor:
                      selectedAddress?._id === addr._id ? "#ffefefff" : "#fff",
                    borderColor:
                      selectedAddress?._id === addr._id ? "#EF4444" : "#eee",
                    borderWidth: selectedAddress?._id === addr._id ? 1 : 0,
                    borderRadius: 8,
                    margin: 8,
                    marginBottom: selectedAddress?._id === addr._id ? 5 : 0,
                  }}
                >
                  <TouchableOpacity
          ...
          >
            ...
            >
              ...
              <TextInput
                ...
                onChangeText={(t) =>
                  setNewAddress({ ...newAddress, street: t })
                }
              />
              <TextInput
                placeholder="City*"
              ...
              <TextInput
                ...
                onChangeText={(t) =>
                  setNewAddress({ ...newAddress, postalCode: t })
                }
                ...
                {["Home", "Office", "Other"].map((label) => (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      borderWidth: 1,
                      borderColor:
                        newAddress.label === label ? "#2563EB" : "#eee",
                      backgroundColor:
                        newAddress.label === label ? "#EFF6FF" : "#fff",
                      alignItems: "center",
                    }}
                  ...
                  >
                    <Text
                      style={{
                        color: newAddress.label === label ? "#2563EB" : "#666",
                      }}
                    >
                      {label}
                    </Text>
