...
export default function SingleBookingScreen() {
    ...
    const setupSocket = async () => {
      ...
      if (data.bookingId === id || data._id === id) {
        ...
        Toast.show({
          ...
          text2: STATUS_STEPS.find(s => s.key === data.status)?.title || 'Booking updated'
        });
      }
    };
  ...
  if (loading && !refreshing) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center', paddingBottom: insets.bottom }}>
        <ActivityIndicator size="large" color="#cc2e2e" />
        <Text style={{ marginTop: 16, fontSize: 16, color: '#666' }}>Loading your booking...</Text>
      </SafeAreaView>
    );
  }
  ...
  if (error && !booking) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center', padding: 32, paddingBottom: insets.bottom }}>
        <Ionicons name="cloud-offline" size={80} color="#94A3B8" />
        <Text style={{ fontSize: 20, fontWeight: '700', color: '#1E293B', marginTop: 20 }}>Connection Lost</Text>
        <Text style={{ fontSize: 15, color: '#64748B', textAlign: 'center', marginTop: 8 }}>{error}</Text>
        <TouchableOpacity
          onPress={onRefresh}
          style={{ marginTop: 24, backgroundColor: '#cc2e2e', paddingHorizontal: 32, paddingVertical: 14, borderRadius: 30 }}
        >
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC', paddingBottom: insets.bottom }} edges={['top']}>
      ...
      <ScrollView
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#cc2e2e']} />
        }
      ...
      >
        ...
        {booking?.status === 'pending' && (
          <View style={{ backgroundColor: '#fef4e8ff', margin: 16, padding: 16, borderRadius: 12, flexDirection: 'row' }}>
            <MaterialIcons name="pending-actions" size={28} color="#F97316" />
            <Text style={{ marginLeft: 12, fontWeight: '700', color: '#f97416ff' }}>Service partner will be assigned soon!</Text>
          </View>
        )}

        {/* Expired Status Banner */}
        {booking?.status === 'expired' && (
          <View style={{ backgroundColor: '#fff0f0ff', margin: 16, padding: 16, borderRadius: 12, flexDirection: 'row' }}>
            <Entypo name="circle-with-cross" size={28} color="#f91616ff" />
            <Text style={{ marginLeft: 12, fontWeight: '700', color: '#f91616ff' }}>Your booking is expired!</Text>
          </View>
        )}
        ...
        <View style={{ paddingHorizontal: 24, paddingVertical: 32 }}>
          {STATUS_STEPS.map((step, index) => {
            ...
            return (
              <View key={step.key} style={{ flexDirection: 'row', marginBottom: 8 }}>
                {/* Left: Circle + Line */}
                <View style={{ alignItems: 'center', marginRight: 20 }}>
                  ...
                  {index < STATUS_STEPS.length - 1 && (
                    <View style={{
                      width: 2,
                      height: 40,
                      backgroundColor: index < currentStepIndex ? '#cc2e2e' : '#E5E7EB',
                      marginTop: 8,
                    }} />
                  )}
                </View>
                ...
                {/* Right: Text */}
                <View style={{ flex: 1, paddingTop: 2, justifyContent: 'flex-start' }}>
                  <Text style={{
                    fontSize: 14,
                  ...
                  {isCurrent && (
                    <Text style={{ fontSize: 12, color: '#cc2e2e', marginTop: 4, fontWeight: '600' }}>
                      Happening now
                    </Text>
                  )}
        ...
        <View style={{ backgroundColor: '#FFFFFF', marginHorizontal: 2, marginTop: 16, padding: 16, borderRadius: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 2 }}>
          <Text style={{ fontSize: 20, fontWeight: '800', color: '#1A202C', marginBottom: 16 }}>
            Service Details
          </Text>

          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
            <Image
              source={{ uri: booking?.services?.[0]?.serviceId?.imageUrl || 'https://via.placeholder.com/80' }}
              style={{ width: 80, height: 80, borderRadius: 16 }}
            />
            <View style={{ marginLeft: 16, flex: 1 }}>
              <Text style={{ fontSize: 18, fontWeight: '700' }}>{booking?.services?.[0]?.serviceId?.name}</Text>
              <Text style={{ fontSize: 14, fontWeight: '600', color: '#505154ff' }}>{booking?.services?.[0]?.serviceId?.description
}</Text>
              <Text style={{ fontSize: 24, fontWeight: '900', color: '#cc2e2e', marginTop: 8 }}>
          ...
          {booking?.status === 'completed' && (
            <View style={{
              ...
              <Text style={{ fontSize: 20, fontWeight: '800', color: '#065F46', marginTop: 12 }}>
